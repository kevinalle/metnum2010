...
int main(){
	...
	tierra.m = PP*1.e-6; // 10^24 Kg = m_sol/10^6

	Planeta luna;
	luna.x = Vector(-7.248557601516714e-01, -7.039097848869444e-01, 1.029051462977911e-04);
	luna.v = Vector(1.220576787081027e-02, -1.211595432894550e-02, 4.775108508860639e-05);
	luna.m = PP*1.e-10; // 10^20 Kg = m_sol/10^10

	Observador o;
//	o.pos = Vector(0,0,0);
	o.dir = Vector(0,1,0).normalize();//o.pos.inverse().normalize();
	o.up = Vector(0,0,1).normalize();
	...
	while(!quit){
		...
		draw(screen,depth,o,luna.x,255,0,255);
